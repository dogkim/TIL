# computer/ 이력

컴퓨터공학 전반을 다루는 상위 카테고리. 여러 세션에 걸쳐 하위 폴더별로 대대적 재정리 진행 — 폴더명 영어화, `NN-Title-Case.md` 파일명 통일, 이미지 `assets/` 정리, 시험이 끝난 과목(Algorithm, Automata 등)은 강의 필기의 빈 부분을 CLRS 등 표준 지식으로 채워 완성.

주요 소스:
- `TIL-Legacy`, 구 OneDrive `TIL` 폴더의 `computer-engineering` — 마이그레이션 후 원본 삭제.
- OneDrive `CS,CE` 폴더(3-1/3-1 겨울/3-2 수업 자료) — `Java/`, `Programming-Basics/06~07` 신설. 원본은 보존.

각 하위 폴더의 구체적 경위는 폴더별 `_History.md` 참고.
